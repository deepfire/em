...
(use-package org
  ...
  (bind-key "C-o"           (lambda ()
                              ...
                              (other-window 1))
                                                    org-mode-map)
  (bind-key "C-c C-x x"     (lambda ()
                              (interactive)
                              (org-ctrl-c-ctrl-c)
                              (org-redisplay-inline-images))
                                                    org-mode-map)
  (defun workbench-render-report ()
    (interactive)
    (org-table-recalculate-buffer-tables)
    (org-table-recalculate-buffer-tables)
    (org-babel-execute-buffer)
    (org-redisplay-inline-images))
  (bind-key "C-c C-x C-x"  'workbench-render-report org-mode-map)
...
With optional argument MARKERP, return the position as a new marker."
    ...
    (let ((remote-match (org-id-parse-remote-table-ref id)))
      (if remote-match
          (org-id-find-remote (first remote-match) (second remote-match) markerp)
